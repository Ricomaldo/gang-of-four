---
title: 'GANG — Copy deck (la voix)'
created: '2026-07-10'
updated: '2026-07-10'
version: 0.1.0
status: draft
type: copy-deck
---

# GANG — Copy deck (la voix)

> **SCRATCH — 1er jet du 10/07.** La vraie langue est celle de la bande d'Eric ;
> ces chaînes sont des **propositions à réécrire**. 2ᵉ passe à venir.
>
> **Discipline (pilier 1) :** aucune chaîne ne nomme l'imagerie — pas de « capo »,
> pas de « parrain ». La swagger tient dans le **ton**, le mot **GANG**, et les
> emoji. Le jour où l'UI écrit « le Parrain », on l'a perdu.

## Deux registres

- **permanent · calme** — la ligne de standing, toujours là ; elle *murmure* la
  hiérarchie, elle ne proclame pas.
- **ponctuel · fort** — les beats, torse bombé, **rares**.

## Trois scopes — ne jamais confondre

- **manche** (le flash) · **partie** (un jeu, remis à zéro) · **gang** (permanent,
  inter-sessions).

## Le champion — chemin A validé

Label **plat + ✌️**. Pas de titre malin. Le ✌️ et le ton portent la swagger.

## Les chaînes (propositions)

| Moment | Scope | Registre | Chaîne |
|---|---|---|---|
| Qui mène la partie (continu) | partie | permanent · calme | **{prénom} mène** |
| Égalité en tête | partie | permanent · calme | *ex æquo* |
| Carré joué (anim frime) | manche | ponctuel · fort | **GANG !** *(cri de la table, en écho)* |
| Manche pliée | manche | ponctuel · fort | **{prénom} plie la manche** |
| Branlée (≥30 sur la manche) | manche | ponctuel · fort | **{prénom} colle une branlée** |
| Le trône change | partie | ponctuel · fort | **{prénom} passe devant** |
| Fin de partie — vainqueur | partie | ponctuel · fort | **{prénom} gagne la partie** |
| Fin de partie — dernier | partie | ponctuel · fort | **{prénom} ramasse le 💩** |
| Palmarès — champion | gang | — | **le gang est à {prénom}** |
| Palmarès — looser | gang | — | **{prénom} traîne le 🐌** |

## Notes

- *« le gang est à {prénom} »* = **gang scope uniquement** (jamais la fin d'une
  partie — le vainqueur d'un soir n'a pas le gang).
- Le **flash** (GANG !, manche pliée) et le **règne** (mène, passe devant) ne
  parlent pas la même langue — on peut gagner la partie sans gagner de manche.
- La branlée n'a pas encore d'emoji ni de rendu — **pilier 4, ouvert.**
