...
- _Tranché (31/07) :_ pas de hint — silence assumé, cohérent avec le style épuré du placard.
...
- _Caduc (31/07) :_ le composant a changé de forme depuis (`CenterDisc` → `Hub` → `Gong`) ; `Gong.tsx` n'affiche plus aucun label d'état, tap-only. Le wording « START GAME ? » et sa vérification simu ne s'appliquent plus à l'écran actuel.
