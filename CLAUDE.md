...
**Phase courante (18/07) : alpha-signature 0.2 — la signature PLACARD posée sur les 5 écrans.** Le socle 0.2 (signé le 12/07) a été **bâti par lots** le 13/07 (boucle supervision/intégration, `journal/2026-07-13-brief-lot-*`, chaque lot en **porte ②** puis scellé : domaine · scène · voix · moyeu · mémoire · ménage+undo · frime). Puis, le **18/07, le vernis visuel — le PLACARD — implémenté** sur les 5 écrans via le **canal direct Claude Design → Claude Code** (`/design-login`, spec précise) : fontes (Anton/Plex Mono/Caveat chargées au boot), palette placard, meneur rouge, le gravé, marques **‡ / ‡‡** (supersede l'encoche `/` `//`). **107 tests verts, tsc clean, vérifié sur device (Anton monte).** L'app est **renommée `GANG`**. **Reste :** des **retouches** (grille de score + polish), les **sons** (Jacques), le **ship** (rebuild + page + bump `app.json 0.1.0→0.2.0`, instance build). Fait-foi visuel : `claude_design/2-alpha-signature/retour/specs-placard.md` (intention) + `theme/tokens.ts` (valeurs). Détail : `journal/2026-07-18-placard-implemente.md` (+ `2026-07-13-transition-alpha-signature.md`). Versioning par ère : **`0.1.x`** = alpha-core (tag git `v0.1`) · **`0.2.x`** = alpha-signature.

On est **en forge**, plancher éprouvé et **alpha-core diffusée** aux potes (APK auto-hébergé, `dev.irimwebforge.com`, ex-`GoF Companion`) ; l'**alpha-signature (`GANG`) est bâtie et sa signature placard posée**, elle partira au prochain rebuild. La première **vraie partie à 4** — le test du jalon « tenir la partie » — se joue à la première soirée réelle. Le fil avec Damien et la table est vivant ; le détail vit dans le socle (ci-dessous), pas ici.
